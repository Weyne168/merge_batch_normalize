...
def load_and_fill_biases(src_model, src_weights, dst_model, dst_weights):
    ...
    for i, layer in enumerate(model.layer):
        ...
        if layer.type == 'InnerProduct':
            if layer.inner_product_param.bias_term == False:
                layer.inner_product_param.bias_term = True
                layer.inner_product_param.bias_filler.type = 'constant'
                layer.inner_product_param.bias_filler.value = 0.0
...
def merge_conv_and_bn(net, i_conv, i_bn, i_scale):
    ...
    if net.layers[i_conv].type == 'Convolution' or net.layers[i_conv].type == 'InnerProduct':
...
def merge_batchnorms_in_net(net):
    ...
    for i, layer in enumerate(net.layers):
        ...
        assert (len(l_top) == 1)
        l_top = l_top[0]
        print('merging %s into %s...' % (l_name, l_bottom))

        can_be_absorbed = True

        # find the index of bottom layer
        ...
        # find the index of following Scale
        scale_ind = None
        ...
        if zero_mean and one_var and length_is_1:
            ...
            for j in xrange(i + 1, len(model.layer)):
                ...
                if l_top in bottoms_of_j:
                    ...
                    if model.layer[j].type in ['ReLU', 'Sigmoid', 'Eltwise', 'PReLU']:
                        active_ind = j
            ...
            if active_ind is not None:
                ...
                if act_layer.name not in acts:
                    acts[act_layer.name] = 0
                else:
                    acts[act_layer.name] += 1

                act_layer.bottom[acts[act_layer.name]] = l_bottom
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='/home/guoweiyu/workspace/reid_attr.prototxt',
                        help="The net definition prototxt")
    parser.add_argument('--weights', type=str, default='/home/guoweiyu/workspace/reid_attr.caffemodel',
